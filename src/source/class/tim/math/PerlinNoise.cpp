...
#include <algorithm>
#include <cstdlib>
unsigned char PerlinNoise::permutation[256]={ 151, 160, 137, 91, 90, 15, 131, 13, 201,
...
void PerlinNoise::init(unsigned seed) {
	...
	for (int i = 0; i < length; i++) {
		p.at(i) = permutation[rand() % 256];
	}
}
double PerlinNoise::noise(double x, double y, double z,double sample_range) {
	x*=(255.0*sample_range);
	y*=(255.0*sample_range);
	z*=(255.0*sample_range);
	int X = (int) floor(x) & 255;       // FIND UNIT CUBE THAT CONTAINS POINT.
	int Y = (int) floor(y) & 255;       //mask the last 8 bit
	int Z = (int) floor(z) & 255;		//(limit value range from0~255
	...
	int BA = p.at(B) + Z;
	int BB = p.at(B + 1) + Z;
	//if(A>511||AA>511||AB>511||B>511||BA>511||BB>511){
